...
def LinearGradientDescent(
    fun, X, y, W, n_iters, lr=0.01, l1_ratio=0.0, l2_ratio=0.0, metric=None, predFn=None
):
    history = []
...
def LinearStochasticGradientDescent(
    fun,
    X,
    y,
    W,
        ...
        for j in range(len(X) // batch_size):
            ...
            if np.isnan(W).sum() > 0:
                raise ArithmeticError(
                    "Nan encountered in W at epoch " + str(j) + " iteration " + str(i)
                )
        if metric is not None:
            history.append(metric(y, predFn(X, W)))
    return W, history
